...
function [dydt,bflow,MR,P,U,uprel,F,epi,sex] = exerciseMod(t,y,WR,ins0,glu0,c0,tstart,tend,sex)
    ...
    if sex == 0 % male
        ...
        WG = 2.0;    % GI tract
        WL = 1.8;    % Liver
        WA = 11;     % Adipose tissue, 16% body fat content; set WA=70*0.295 to match female adiposity
        WO = Wbody - WB - WH -WM - WG - WL - WA; % Others
    else % female
        % organ weights [whole body = 58kg]
        Wbody = 58; 
        WB = 1.2;    % Brain
        WH = 0.253;  % Heart
        WM = 17*0.8; % Skeletal muscles, excluding upper extremities which account for 18-20%
        WG = 2.0;    % GI tract
        WL = 1.4;    % Liver
        WA = 17.1;   % Adipose tissue, 29.5% body fat content  
        WO = Wbody - WB - WH -WM - WG - WL - WA; % Others
    end
    ...
    if sex == 0 % male
        ...
        Q(5)=0.25; % chosen so that Q4 + Q5 = 1.35 (females);
...
% Uptake-release rates for the Muscle (M) compartment - y3(x)
    ...
    if sex == 0 % male
        ...
        s(1) = 9.8472;  %GLC vs ----> male value = 10.03472
        s(2) = 1.3530;  %PYR vs ----> male value = 1.3009
        s(3) = 0.5596;  %LAC vs ----> male value = 0.5725
        s(4) = 0.1819;  %ALA
        s(5) = 1.1458;  %GLR
...
%Uptake-release rates for the Liver (L) compartment - y5(x)  
    ...
    if sex == 0 % male
        ...
        s(1) = 0.6744; %GLC vs ----> male value = 0.6796
        s(2) = 0.1838; %PYR
        s(3) = 0.6098; %LAC vs ----> male value = 0.6341
        s(4) = -0.1958;%ALA vs ----> male value = -0.10; 
        s(5) = -0.015; %GLR vs ----> male value = 0.0481
...
%Uptake-release rates for the Adipose (A) compartment - y6(x)  
    ...
    if sex == 0 % male
        ...
        s(1) = 1.8915;    %GLC vs ----> male value = 1.9269
        s(2) = 0.1838;    %PYR 
        s(3) = 1.0434;    %LAC
        s(4) = 0.0;       %ALA
        s(5) = 1.5429;    %GLR
        s(6) = 2.3616;    %FFA vs ----> male value = 2.1861 
        s(7) = 8.8328e-4; %TGL vs ----> male value = 0.0009439; 
        s(8) = 249.9761; %O2  
        s(9) = 1.4560;  %CO2
    end
    s(10:22) = 0.0;
...
%Uptake-release rates for the Other tissues (O) compartment - y7(x) 
    ...
    [F1,P1,U1] = brain(t,y1,MR,sex);
    P(1,1:22)=P1(1:22); U(1,1:22)=U1(1:22);
    
    [F2,P2,U2] = heart(t,tstart,y2,WR,epi,epi0,MR,RMR_H,c0,sex);
    P(2,1:22)=P2(1:22); U(2,1:22)=U2(1:22); 
    
    [F3,P3,U3] = muscle(t,tstart,y3,WR,MR,RMR_M,epi,epi0,c0,sex);
    P(3,1:22)=P3(1:22); U(3,1:22)=U3(1:22); 
    
    [F4,P4,U4] = gi(t,tstart,y4,WR,MR,GIR,epi,epi0,c0,sex);
    P(4,1:22)=P4(1:22); U(4,1:22)=U4(1:22); 
    
    [F5,P5,U5] = liver(t,tstart,y5,WR,GIR,MR,c0,sex);
    P(5,1:22)=P5(1:22); U(5,1:22)=U5(1:22); 
    
    [F6,P6,U6] = adipose(t,tstart,y6,WR,MR,GIR,epi,epi0,c0,sex);
